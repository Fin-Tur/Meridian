...
namespace monte_carlo {
    ...
    // ---- rng: thread-local, one instance per thread ----
    static thread_local std::mt19937 rng = [](){
        std::seed_seq seq {
            static_cast<uint64_t>(std::chrono::high_resolution_clock::now().time_since_epoch().count()),
            static_cast<uint64_t>(std::hash<std::thread::id>{}(std::this_thread::get_id())),
            static_cast<uint64_t>(reinterpret_cast<uintptr_t>(&seq)) //Stack address for extra entropy
        };
    ...
    std::unordered_map<regimes, double> regimes_df_multiplier = {
        {regimes::CALM,    1.5},
        {regimes::ENERGIC, 1.0},
        {regimes::CRISIS,  0.5}
    };
    ...
    // ---- generate_sim_preset ----
    sim_preset generate_sim_preset(std::vector<assets::asset>& assets, const std::vector<double>& weights, size_t n_sims, size_t horizon_days, const sim_config& config, double portfolio_value){
        sim_preset preset;
        ...
        for (size_t i = 0; i < assets.size(); i++) {
            ...
            //EWMA to simulate drift-phases : lambda = 0.94 (JP Morgan 1994) | 0.9 for crypto since faster market changes
            ...
            //Avoid extremely low dof which can cause instability in the t-distribution. Fat Tails only important for low horizons
